...
export function EventItem({ event }: EventItemProps) {
  ...
  return (
    <div className="flex items-center gap-2 px-3 py-1.5 text-xs hover:bg-gray-50 dark:hover:bg-gray-700/50">
      <span className={`w-2 h-2 rounded-full shrink-0 ${color}`} />
      <span className="font-medium text-gray-700 dark:text-gray-300 w-24 truncate">{label}</span>
      {name && <span className="text-gray-500 dark:text-gray-400 truncate">{name}</span>}
