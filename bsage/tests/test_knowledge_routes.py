...
class TestKnowledgeSOTEndpoint:
    """Tests for GET /api/knowledge/sot."""

    def test_returns_sot_notes_default(self, client):
        """Default request returns fact and insight notes (SOT)."""
        resp = client.get("/api/knowledge/sot")
        ...
        # Should include facts + insights (3 total), not ideas or tasks
        assert data["total"] == 3
        assert data["page"] == 1
        assert data["page_size"] == 20

    def test_sot_items_have_required_fields(self, client):
        """Each item has title, content, tags, links, updated_at."""
        resp = client.get("/api/knowledge/sot")
        data = resp.json()
    ...
    def test_sot_filter_by_tags(self, client):
        """Filtering by tags returns matching notes only."""
        resp = client.get("/api/knowledge/sot", params={"tags": "python"})
        data = resp.json()
        assert data["total"] == 1
        assert data["items"][0]["title"] == "Python Typing Best Practices"

    def test_sot_filter_by_multiple_tags(self, client):
        """Comma-separated tags filter (OR logic)."""
        resp = client.get("/api/knowledge/sot", params={"tags": "python,weekly"})
        data = resp.json()
        assert data["total"] == 2

    def test_sot_filter_by_project_id(self, client):
        """project_id filters by related links containing the project."""
        resp = client.get("/api/knowledge/sot", params={"project_id": "Project Alpha"})
        data = resp.json()
        assert data["total"] == 2
    ...
    def test_sot_pagination(self, client):
        """Pagination returns correct page slices."""
        resp = client.get("/api/knowledge/sot", params={"page": 1, "page_size": 2})
        data = resp.json()
    ...
    def test_sot_pagination_page_2(self, client):
        """Second page returns remaining items."""
        resp = client.get("/api/knowledge/sot", params={"page": 2, "page_size": 2})
        data = resp.json()
        assert len(data["items"]) == 1
        assert data["total"] == 3

    def test_sot_empty_results(self, client):
        """Non-matching filter returns empty items with total 0."""
        resp = client.get("/api/knowledge/sot", params={"tags": "nonexistent"})
        data = resp.json()
        assert data["total"] == 0
        assert data["items"] == []

    def test_sot_type_sop_returns_tasks(self, client):
        """type=sop returns task notes instead of fact/insight."""
        resp = client.get("/api/knowledge/sot", params={"type": "sop"})
        data = resp.json()
        assert data["total"] == 1
        assert data["items"][0]["title"] == "Deploy Pipeline Setup"

    def test_sot_combined_filters(self, client):
        """Combining project_id and tags narrows results."""
        resp = client.get(
            "/api/knowledge/sot",
            params={"project_id": "Project Alpha", "tags": "python"},
        )
        data = resp.json()
        assert data["total"] == 1
        assert data["items"][0]["title"] == "Python Typing Best Practices"
